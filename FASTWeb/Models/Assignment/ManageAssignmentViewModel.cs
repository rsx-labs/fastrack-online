...
namespace FASTWeb.Models.Assignment
{
    public class ManageAssignmentViewModel
    {
        FASTService.Process.EmployeeManagementProcess empProcess = new EmployeeManagementProcess();
        ...
        private string GetEmployee(int id)
        {
            if (id > 0)
            {
                FASTService.Model.EmployeeModel employee = empProcess.GetEmployeeProfile(id);
        ...
        {
            vwAssetAssignment assignment = assignProcess.GetAssignmentByID(assignID);
